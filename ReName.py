...
def output_wuxing(year, month, day, hour):
    ...
    print("[*] 生辰：%s年%s月%s日, %s时" % (year, month, day, hour))
...
async def filter_name(surname, gender, attr, queue):
    """
    ...
    while 1:
        if match_count == 5:
            break
        name_item = name_tuple[randint(0, len(name_tuple))]
        name = name_item[0]
        full_name = surname + name
        # Match gender and general name word.
        if gender != ngender.guess(full_name)[0][0].upper() or not db_obj.match_name_word(name):
            continue
        score = await name_score(full_name, sur_type)
        if score > 95:
            queue.put_nowait(full_name)
            print("[-] 候选名字：%s" % full_name)
            match_count += 1
            await output_name(name, queue)


async def output_name(name, queue):
    """
    Output names to csv file.
    :param name: name.
    :param queue: name queue.
    :return:
    """
    ...
    while not queue.empty():
        full_name = await queue.get()
        try:
            name_source = db_obj.get_name_source(name)
            with open(OUTPUT_PATH, 'a+', newline='') as fh:
                csv_f = csv.writer(fh)
                csv_f.writerow([full_name, name_source[2], name_source[1] + '(' + name_source[0] + ')', name_source[3]])
...
if __name__ == '__main__':
    ...
    task = loop.create_task(filter_name(args.s, args.g, attr_list, name_queue))
